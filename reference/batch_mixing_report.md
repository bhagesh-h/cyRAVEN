...
- group_col:
  ...
  one or more columns to test batch against; one row of the confounding
  table per column. Pass the study group AND the timepoint on a
  repeated-measures design: the visit is the primary comparison there,
  and checking only the study group leaves it unexamined.
