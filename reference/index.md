...
transferred from fixed coordinates.
...
How far a threshold can move, and what that costs the frequency it
produced. Each cut is resampled and re-derived over the settings that
placed it, and the resulting spread is propagated to every population
that reads it. None of this changes a reported number; it puts a second
number beside it.
