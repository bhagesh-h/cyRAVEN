...
cl <- data.frame(sample = condor@anno$sample_id,
                 event  = condor@anno$event_index,
                 cluster = paste0("k", condor@clustering$FlowSOM$metacluster))
write.csv(cl, "labels.csv", row.names = FALSE)
```
