...
Every caveat in one place, and the methods deliberately left out with
the reason for each.

Read this before quoting a number from a run. Nothing here is
hypothetical: each limitation is something the pipeline can do to a real
result, and each exclusion is a decision that has been argued rather
than an oversight.
...
which it should be revisited. Where a method is required now, the [Using
cyRAVEN with
cyCONDOR](https://bhagesh-h.github.io/cyRAVEN/articles/advanced.html#using-cyraven-with-cycondor)
...
Absent for the reason in section 1 of the
