...
```
  sample A        |                sample B     |
   ...
   ------------------ brightness  ------------------ brightness
        ...
        A's dip                      B's dip
                  ^
        A's cut, copied onto B, lands mid-hump
```
...
```
  SSC (grainy)
    ^
    |            granulocytes   <- grainiest, packed with granules
    |         monocytes
    |    lymphocytes            <- small and smooth
    | debris
    +-------------------------> FSC (size)
```
...
```
      one cell              two stuck together

        _/\_                  _/\__/\_
       /    \                /        \

   H = peak height        similar H
   A = area under it      roughly doubled A
   W = how long it took   wider W
```
