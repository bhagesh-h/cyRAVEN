...
Reference for every file a run writes. A default run on eight samples produces 25
...
`--no-miflowcyt` skips it. `--write-config` and `--write-sample-map` terminate
after writing and cannot be combined with a full run.
