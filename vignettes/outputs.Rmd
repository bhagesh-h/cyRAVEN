...
| `report.html` | Every output above, in the order this documentation says to read them |
...
`report.html` is the reading order made navigable: each section states the
question it answers, shows the figures behind it, and links the tables. A run
that excluded samples says so above every result. It references the figures
beside it rather than embedding them, so it is read in the results directory.

`--no-miflowcyt` and `--no-report` skip those two. `--write-config` and
`--write-sample-map` terminate after writing and cannot be combined with a full
run.
