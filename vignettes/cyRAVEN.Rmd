...
Two routes to the same image. Which you want turns on one question: **are you
going to change the source?**
...
anything you intend to publish: `latest` moving under you is the reproducibility
...
[the gating specification](cyRAVEN.html#the-gating-specification); thresholds, colours and metadata dictionaries in
...
| Counts exported by the counting instrument as its own sheet | `--absolute-counts` reads that file's own layout; see the [Every output file](advanced.html#every-output-file). Fold it into the sheet only if transcribing it is easy. |

### 5. Worked example

Setup is in [Setup](#setup); this is the sequence once the image is in place. It
is the same five steps for any cohort.
...
**2. Fill in `patient_id`, the study group, and any variable you want to test.**
An edit, not a command. Every reserved column is already present and blank;
...
Edit `populations:` to match the panel. `--check` lists the marker names the
files actually carry, which is what a population has to use.
...
The [Diagnostics](advanced.html#diagnostics-in-reading-order) covers how to read the result.
...
See [Setup](#setup). Pull for a released run, build for a modified source tree,
and pin the version for anything you intend to publish.
...
The two input files are described in the [Inputs](cyRAVEN.html#inputs-the-sample-sheet-and-the-config).
...
See [A complete run in five minutes](#a-complete-run-in-five-minutes). Nothing is
downloaded: the data ship inside a package cyRAVEN already depends on, so the
example is reproducible offline.
...
`--samples`. See the [Inputs](cyRAVEN.html#inputs-the-sample-sheet-and-the-config).
