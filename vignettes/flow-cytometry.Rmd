---
title: "About flow cytometry"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{About flow cytometry}
...
What an event is, what a cut is, why `SSC-A` sits in a population definition
beside two antibodies, and what the `-A` on a channel name means.

Every other chapter assumes this one. Nothing here is specific to cyRAVEN, so
skip it if you already work with flow data.
...
- [Gating specification](cyRAVEN.html#the-gating-specification): the full syntax, functional blocks, ratios
  and per-sample overrides.
- [Diagnostics](advanced.html#diagnostics-in-reading-order): the checks that tell you a cut went wrong, in
  the order to read them.
- [Worked example](gallery.html): every figure from a real run, including the
  gating QC panels where you can see cuts sitting where they should.
- [Explore mode](advanced.html#explore-mode-unsupervised-discovery): finding populations without declaring anything,
