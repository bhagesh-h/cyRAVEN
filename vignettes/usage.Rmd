...
cyraven:1.1.0 --help` prints the list at the version installed.
...
`cyraven:1.1.0`. If you skip it, substitute `bhagesh/cyraven:1.1.0` wherever
`cyraven:1.1.0` appears below.
...
docker run --rm -v "$PWD:/src:ro" -v "$PWD/data:/data:ro" \
  ...
  cyraven:1.1.0 --dir /data/fcs --outdir /results
```
...
```bash
docker run --rm $D -v "$PWD/beads:/beads:ro" cyraven:1.1.0 \
  --dir /data/fcs --samples /data/samples.csv --config /data/analysis.yaml \
...
| `--no-marker-group-umaps` | off | Skip `marker_umaps_by_group/`, one full-size UMAP per marker. Written by every run: split by group where a group column resolves to two or more groups, one panel per marker where it does not. Turn it off for a panel wide enough that one PNG per marker is a folder you do not want |
